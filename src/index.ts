...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'GeoNames');
}
...
async function searchPlaces(username: string, query: string, country?: string, limit?: number) {
  ...
  const params = new URLSearchParams({
    ...
    username,
    type: 'json',
    // Without these, GeoNames' `q` fuzzy-matches across ALL attributes and orders
    // by an opaque relevance — so "Paris" (US) returned Annapolis first, and the
    // real Paris, TX ranked below tiny Paris, AR. isNameRequired drops matches
    // whose name doesn't contain the term (kills the fuzzy junk); orderby=
    // population surfaces the prominent city first (Paris, TX > Paris, AR), which
    // is what "what state is <city> in" wants.
    isNameRequired: 'true',
    orderby: 'population',
  });
  if (country) params.set('country', country.toUpperCase());

  const res = await pwFetch(`${BASE_URL}/searchJSON?${params}`);
  if (!res.ok) throw await httpError(res, 'GeoNames API error');
  ...
  // Re-rank so the FIRST result is the place people mean. GeoNames' population
  // order alone surfaces bi-state CBSA aggregates (fcode "economic region",
  // e.g. "Portland-Vancouver-Hillsboro, OR-WA Metro Area") ABOVE the core city,
  // and their admin_region can be the wrong state (Portland → Washington, not
  // Oregon). And fuzzy `q` still returns non-matching names (Annapolis for
  // "Paris"). Priority: (1) name actually contains the query term — drops the
  // fuzzy junk; (2) a real settlement over an economic-region aggregate — so the
  // core city's state wins over a multi-state metro; (3) population.
  const q = query.toLowerCase().trim();
  const nameMatch = (p: ReturnType<typeof formatPlace>) =>
    `${p.name ?? ''} ${p.toponym_name ?? ''}`.toLowerCase().includes(q);
  const isAggregate = (p: ReturnType<typeof formatPlace>) =>
    (p.feature_code ?? '').toLowerCase().includes('economic region') ||
    /\b(metro|micro|metropolitan|statistical)\b.*\barea\b/i.test(p.name ?? '');
  const places = (data.geonames ?? []).map(formatPlace).sort((a, b) => {
    const am = nameMatch(a) ? 1 : 0;
    const bm = nameMatch(b) ? 1 : 0;
    if (am !== bm) return bm - am;
    const aa = isAggregate(a) ? 1 : 0;
    const ba = isAggregate(b) ? 1 : 0;
...
async function getNearby(username: string, lat: number, lng: number, radius?: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/findNearbyJSON?${params}`);
  if (!res.ok) throw await httpError(res, 'GeoNames API error');
...
async function getTimezone(username: string, lat: number, lng: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/timezoneJSON?${params}`);
  if (!res.ok) throw await httpError(res, 'GeoNames API error');
...
async function findPostalCodes(
  ...
  const res = await pwFetch(`${BASE_URL}/${endpoint}?${params}`);
  if (!res.ok) throw await httpError(res, 'GeoNames API error');
